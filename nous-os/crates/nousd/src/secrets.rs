...
impl Secrets {
    ...
    fn merge_text(&mut self, text: &str) {
        for line in text.lines() {
            ...
            if let Some((k, v)) = line.split_once('=') {
                ...
                if !v.is_empty() {
                    self.keys.insert(k.trim().to_ascii_lowercase(), v.to_string());
                }
            }
        }
    }
    ...
    pub fn set_at(path: &std::path::Path, provider: &str, key: &str) -> Result<(), String> {
        ...
        existing.keys.insert(provider.to_ascii_lowercase(), key.trim().to_string());
...
pub fn redact(text: &str) -> String {
    ...
    for word in text.split_inclusive(char::is_whitespace) {
        ...
        let looks_like_a_key = t.len() > 20
            ...
            || (t.len() > 32 && t.chars().all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '_')
...
mod tests {
    ...
    fn stores_keys_owner_only() {
        ...
        assert_eq!(mode, 0o600, "credentials must not be group or world readable");
        let dir_mode =
            std::fs::metadata(path.parent().unwrap()).unwrap().permissions().mode() & 0o777;
        assert_eq!(dir_mode, 0o700, "the containing directory must be private too");
    ...
    fn round_trips_several_providers() {
        ...
        assert_eq!(s.get("openai"), Some("sk-openai-2"), "storing a second key must not drop the first");
    ...
    fn ollama_counts_as_configured_without_a_key() {
        ...
        let ollama = providers.iter().find(|p| p.str_or("provider", "") == "ollama").unwrap();
        assert!(ollama.bool_or("configured", false));
        assert!(!ollama.bool_or("needs_key", true));
        let anthropic = providers.iter().find(|p| p.str_or("provider", "") == "anthropic").unwrap();
        assert!(!anthropic.bool_or("configured", true), "no key means not configured");
    }
    ...
    fn comments_and_blank_lines_are_ignored() {
        ...
        assert!(s.get("empty").is_none(), "an empty value is not a credential");
    }
    ...
    fn the_secrets_path_is_on_the_protected_read_list() {
        ...
        let cap = nous_core::Capability::parse(&format!("fs.read:{}", path)).unwrap();
        assert!(
            nous_core::cap::protected_violation(&cap).is_some(),
    ...
    fn redaction_hides_keys_but_keeps_prose() {
        ...
        assert_eq!(redact("a short note about files"), "a short note about files");
    }
}
